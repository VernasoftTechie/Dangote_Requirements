"! <p class="shorttext synchronized">BP Customer ICF API - processing log &amp; reprocess</p>
"! Displays ZBP_CUST_LOG as an ALV grid ( date / time, external id, status,
...
REPORT zbp_cust_log_report.

TYPE-POOLS icon.
TABLES zbp_cust_log.

TYPES: BEGIN OF ty_row,
         log_id         TYPE zbp_cust_log-log_id,
         created_date   TYPE d,
         created_time   TYPE t,
         operation      TYPE zbp_cust_log-operation,
         customer_id    TYPE zbp_cust_log-customer_id,
         application_id TYPE zbp_cust_log-application_id,
         status         TYPE zbp_cust_log-status,
         status_icon    TYPE icon_d,
         http_status    TYPE zbp_cust_log-http_status,
         partner        TYPE zbp_cust_log-partner,
         customer       TYPE zbp_cust_log-customer,
         retry_count    TYPE zbp_cust_log-retry_count,
         message        TYPE zbp_cust_log-message,
         created_by     TYPE zbp_cust_log-created_by,
       END OF ty_row.
...
SELECT-OPTIONS s_cust FOR zbp_cust_log-customer_id.
SELECT-OPTIONS s_stat FOR zbp_cust_log-status.
SELECT-OPTIONS s_op   FOR zbp_cust_log-operation.
SELECTION-SCREEN END OF BLOCK b1.

SELECTION-SCREEN BEGIN OF BLOCK b2 WITH FRAME TITLE TEXT-b02.
PARAMETERS p_logid TYPE zbp_cust_log-log_id.
...
CLASS lcl_app DEFINITION FINAL.
  ...
  PRIVATE SECTION.
    ...
    METHODS select_rows.
    METHODS show_alv.
    METHODS show_messages IMPORTING iv_log_id TYPE zbp_cust_log-log_id.
ENDCLASS.
...
CLASS lcl_app IMPLEMENTATION.
  ...
  METHOD reprocess.
    DATA lt_id TYPE STANDARD TABLE OF zbp_cust_log-log_id.
    ...
    DATA(ls_sum) = zcl_bp_cust_log=>reprocess_multi( lt_id ).
    MESSAGE s015(zbp_cust_msg) WITH ls_sum-total ls_sum-succeeded ls_sum-failed.
  ENDMETHOD.
    ...
    LOOP AT lt_log INTO DATA(ls).
      ...
      APPEND VALUE ty_row(
        ...
        status_icon    = SWITCH #( ls-status
                           WHEN zif_bp_cust_types=>c_log_status-success     THEN icon_green_light
                           WHEN zif_bp_cust_types=>c_log_status-reprocessed THEN icon_green_light
                           WHEN zif_bp_cust_types=>c_log_status-pending     THEN icon_yellow_light
                           ELSE icon_red_light )
  ...
  METHOD show_alv.
    ...
    TRY.
        ...
        lo_alv->get_display_settings( )->set_list_header( |BP Customer ICF API - processing log| ).
