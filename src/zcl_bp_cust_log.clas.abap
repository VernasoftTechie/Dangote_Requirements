"! <p class="shorttext synchronized">BP Customer ICF API - processing log</p>
"! Persists every inbound call ( success and failure ) in ZBP_CUST_LOG /
"! ZBP_CUST_LOG_MSG so that failed requests can be analysed in report
"! ZBP_CUST_LOG_REPORT and re-triggered from the stored payload.
CLASS zcl_bp_cust_log DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS record
      IMPORTING iv_operation    TYPE zbp_cust_log-operation
                iv_customer_id  TYPE zbp_cust_id
                iv_application_id TYPE zbp_cust_appid OPTIONAL
                iv_ext_status   TYPE zbp_cust_status OPTIONAL
                iv_status       TYPE zbp_cust_flag
                iv_http_status  TYPE i
                iv_raw_json     TYPE string OPTIONAL
                is_result       TYPE zbp_cust_s_create_res OPTIONAL
                it_messages     TYPE zbp_cust_t_message OPTIONAL
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS save_messages
      IMPORTING iv_log_id   TYPE sysuuid_c22
                it_messages TYPE zbp_cust_t_message.
ENDCLASS.


CLASS zcl_bp_cust_log IMPLEMENTATION.
  ...
  METHOD record.
    ...
    DATA(ls_log) = VALUE zbp_cust_log(
      log_id         = rv_log_id
      direction      = zif_bp_cust_types=>c_direction-inbound
    ...
    INSERT zbp_cust_log FROM ls_log.
  ...
  METHOD save_messages.
    DATA lt_msg TYPE STANDARD TABLE OF zbp_cust_log_msg.
    DATA lv_seq TYPE zbp_cust_log_msg-seqnr.

    DELETE FROM zbp_cust_log_msg WHERE log_id = @iv_log_id.
    ...
    IF lt_msg IS NOT INITIAL.
      INSERT zbp_cust_log_msg FROM TABLE @lt_msg.
    ENDIF.
  ENDMETHOD.


  METHOD reprocess.
    SELECT SINGLE * FROM zbp_cust_log
      INTO @DATA(ls_log)
      WHERE log_id = @iv_log_id.
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_bp_cust
        MESSAGE e013(zbp_cust_msg) WITH iv_log_id.
    ENDIF.
    IF ls_log-request_json IS INITIAL.
      RAISE EXCEPTION TYPE zcx_bp_cust
        MESSAGE e014(zbp_cust_msg) WITH iv_log_id.
    ENDIF.

    CASE ls_log-operation.
      WHEN zif_bp_cust_types=>c_operation-create.
        DATA ls_req TYPE zbp_cust_s_create_req.
        /ui2/cl_json=>deserialize(
          EXPORTING json        = ls_log-request_json
                    pretty_name = /ui2/cl_json=>pretty_mode-camel_case
          CHANGING  data        = ls_req ).
        rs_result = NEW zcl_bp_cust_create( )->execute(
                      is_request  = ls_req
                      iv_raw_json  = ls_log-request_json
                      iv_write_log = abap_false ).

      WHEN zif_bp_cust_types=>c_operation-read.
        TRY.
            DATA(ls_read) = NEW zcl_bp_cust_read( )->execute( ls_log-customer_id ).
            rs_result = VALUE #( customer_id = ls_log-customer_id
                                 partner     = ls_read-partner
                                 customer    = ls_read-customer
                                 success     = abap_true ).
          CATCH zcx_bp_cust INTO DATA(lx_read).
            rs_result = VALUE #( customer_id = ls_log-customer_id
                                 success     = abap_false
    ...
    ls_log-status       = COND #( WHEN rs_result-success = abap_true
                                  THEN zif_bp_cust_types=>c_log_status-reprocessed
                                  ELSE zif_bp_cust_types=>c_log_status-error ).
    ...
    UPDATE zbp_cust_log FROM ls_log.
